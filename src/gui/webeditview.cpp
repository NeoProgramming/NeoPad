...
static QString convertMarkdownLists(const QString &markdown)
{
	...
	for (const QString &line : lines) {
		...
		if (match.hasMatch()) {
			// Ќайден элемент списка
			currentList.append("<li>" + match.captured(1) + "</li>");
		}
		else {
			// Ќе элемент списка - закрываем предыдущий список если он был
	...
	// «акрываем последний список если он осталс€
	if (!currentList.isEmpty()) {
		result += "<ul>\n" + currentList.join("\n") + "\n</ul>\n";
	}
...
static QString convertCodeBlocks(const QString &markdown) 
{
	...
	// –егул€рное выражение дл€ поиска блоков кода с обратными кавычками
	...
	// —начала собираем все совпадени€
	while (it.hasNext()) {
		matches.append(it.next());
	}

	// ќбрабатываем совпадени€ с конца к началу (чтобы не сбивать позиции)
	for (int i = matches.size() - 1; i >= 0; --i) {
		QRegularExpressionMatch match = matches[i];
		QString lang = match.captured(1).trimmed();
		QString code = match.captured(2);

		// Ёкранируем HTML-символы в коде
		code.replace("&", "&amp;")
			.replace("<", "&lt;")
			.replace(">", "&gt;")
			.replace("\"", "&quot;");

		// ‘ормируем HTML дл€ блока кода
		QString htmlCode;
		if (lang.isEmpty()) {
		...
		// «амен€ем блок кода в исходной строке
...
bool isAlignmentRow(const QStringList& row)
{
    ...
    // ѕровер€ем, состоит ли строка только из символов: |, :, -, пробелы
...
QString convertTableToHtml(const QList<QStringList>& tableRows)
{
    ...
    // ѕерва€ строка - заголовки
    QStringList headers = tableRows[0];

    // ¬тора€ строка - выравнивание (пропускаем еЄ, т.к. форматирование не нужно)
...
QStringList parseTableRowCells(const QString& row)
{
    ...
    // ќчищаем пробелы по кра€м каждой €чейки
...
QString convertMarkdownTables(const QString& text)
{
    ...
    for (int i = 0; i < lines.size(); ++i) {
        ...
        // ѕровер€ем, €вл€етс€ ли строка частью таблицы (начинаетс€ с |)
