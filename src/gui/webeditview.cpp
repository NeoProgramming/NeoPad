...
void WebEditView::onEditPasteAsLWText()
{
	...
	//QRegularExpression regex("([.!?…])(?![»”\"\\p{Pf}])(\\S)");
	//QRegularExpression regex("([.!?…]+[»”\"\\p{Pf}]*)(?!\\s)(\\S)");
	//QRegularExpression regex("([.!?…]{1,3}(?:[»”\"\\p{Pf}])*)(?![\\s\\n])(\\S)");
	//QRegularExpression regex("((?:[.!?]|…)(?:[.!?…»”\"\\p{Pf}])*)(?![\\s\\n])(\\S)");
	//QRegularExpression regex("((?:[.!?]|\\.\\.\\.)(?:[.!…»”\"\\p{Pf}])*)(?![\\s\\n])(\\S)");
	//QRegularExpression regex("((?:[.!?]|\\.\\.\\.|…)(?:[.!…»”\"\\p{Pf}])*)(?![\\s\\n])(\\S)");
	//text = text.replace(regex, "\\1<p>\\2");

	//QRegularExpression regex("([.!?…\\p{P}]+)(?=\\p{L})");
	//QRegularExpression regex("([.!?…,:;\\])}\\p{Pf}]+)(?=\\p{L})");
	//QRegularExpression regex("([.!?…,:;»”\"')\\]}\\p{Pf}]+)(?=\\p{L})");
...
/*
	// 1. Знаки препинания, после которых идет буква
	QRegularExpression regex1("([.!?…,:]+)(?=\\p{L})");
	text = text.replace(regex1, "\\1<p>");

	QChar chars[] = { 0x002D , 0x2010 , 0x2013 , 0x2014 };
	for (auto c : chars) {
		QString pattern2 = QString("([.!?…,:]+)(?=%1)").arg(c);
